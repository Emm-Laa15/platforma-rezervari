...
const RezervareNouaMotoreta = () => {
  const [start, setStart] = useState(new Date());
  const [end, setEnd] = useState(new Date());
  const [success, setSuccess] = useState(false); 
  const [sumaRon, setSumaRon] = useState(0);

  const [product, setProduct] = useState('Motoreta');
  ...
  const handleSubmit = async (event) => {
    ...
    console.log("Tipul produsului este: " + newReservation.motoretaModel)
    ...
    try {
        await axios.post('http://localhost:3001/rezervari/motoreta', newReservation);
  ...
  return (
    ...
    <div className='form-group'>
          <label htmlFor='MotoretaModel'>Model Vehicul</label>
          <select
            name='MotoretaModel'
            id='MotoretaModel'
            value={product}
...
export default RezervareNouaMotoreta;